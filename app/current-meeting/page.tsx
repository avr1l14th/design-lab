...
const asset = (p: string) => `${BASE}/current-meeting/${p}`;
...
// Current-meeting widget — live recording shown in the sidebar.
// A full-width block (bordered top+bottom, subtle fill) holding a single
// row: source icon + meeting title + REC indicator. Sizes/elements 1-to-1
// from Figma nodes 35102:3113 (default), 35187:3243 (hover), 35187:5017 (tooltip).
function RecIndicator() {
  return (
    <span className="relative flex shrink-0 items-center">
      {/* outer ring 10px — border-color flickers (light pink ↔ red-300) */}
      <span
        className="rec-flicker h-[10px] w-[10px] shrink-0 rounded-full border-2 border-solid"
        style={{ backgroundColor: "rgba(204,51,51,0.4)" }}
      />
      {/* solid core 5px, centered on the ring */}
      <span
  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow: `inset 0 1px 0 ${tokens.grey40}, inset 0 -1px 0 ${tokens.grey40}`,
      }}
    >
      <div className="flex w-full items-center gap-[8px] rounded-[3px] p-[4px]">
        {/* 16px slot, 14px icon — matches Figma */}
        <span className="flex h-[16px] w-[16px] shrink-0 items-center justify-center">
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src={asset(sourceIcon)} alt="" className="h-[14px] w-[14px] object-contain" />
        </span>
        <span
          className="min-w-0 flex-1 truncate text-[13px] font-normal leading-[16px]"
          style={{ color: tokens.black, letterSpacing: "-0.13px" }}
        >
          {title}
        </span>
        ...
        <span className="group/stop relative flex h-[16px] w-[34px] shrink-0 items-center">
          <span className="cm-rec-a absolute right-0 top-1/2 flex items-center gap-[4px]">
            ...
            <span
              ...
              style={{ color: tokens.red, letterSpacing: "-0.1px" }}
            >
              REC
            </span>
          </span>
...
export default function SearchFiltersPage() {
  ...
  // Recording stopped from the sidebar widget → widget hides immediately, then the
  // processing meeting appears at the top of the list after a short delay (realism).
  ...
  return (
    <main className={`${inter.className} flex h-screen w-full overflow-hidden bg-white`} style={{ color: tokens.black }}>
      <div className="relative flex h-full flex-1 items-stretch bg-white">
        ...
        <aside
          className="sticky top-0 z-30 flex h-screen w-[280px] shrink-0 flex-col justify-between border-r border-solid pt-[16px] pb-[4px]"
