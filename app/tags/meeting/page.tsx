...
function MeetingTopBar({ onAction }: { onAction: (label: string) => void }) {
  ...
  return (
    <header className="flex h-[54px] shrink-0 items-center justify-between pl-[24px] pr-[24px]">
      <Link
        href="/tags"
        className={`rounded-[3px] text-[13px] leading-[normal] tracking-[-0.13px] hover:text-[#585E6C] ${pressableClass} ${focusRingClass}`}
      ...
      <div className="flex shrink-0 items-center gap-[8px]">
        <div
          className="flex h-[32px] items-center rounded-[4px] border"
          style={{ borderColor: tokens.border }}
        >
          <button
            type="button"
            aria-label="AI-отчет"
            onClick={() => onAction("AI-отчет")}
            className={`h-[30px] w-[30px] rounded-l-[3px] hover:bg-[#F7F7F8] ${pressableClass} ${focusRingClass}`}
          >
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img
              src={gcAsset("mt-ai-30.svg")}
              alt=""
              width={30}
              height={30}
              className="block"
          ...
          <button
            type="button"
            aria-label="Другие отчеты"
            onClick={() => onAction("Список отчетов")}
        ...
        >
          <button
            type="button"
            onClick={() => onAction("Поделиться")}
            className={`${btn} rounded-l-[3px] border-r px-[8px]`}
          ...
          <button
            ...
            onClick={() => onAction("Ссылка скопирована")}
            className={`flex h-[30px] w-[30px] items-center justify-center rounded-r-[3px] bg-white hover:bg-[#F7F7F8] ${pressableClass} ${focusRingClass}`}
            style={{ color: tokens.grey }}
          >
            <Ic name="fig-link" />
        ...
        <button
          type="button"
          onClick={() => onAction("Экспорт")}
          className={`${btn} rounded-[4px] border px-[8px]`}
          style={{ borderColor: tokens.border, color: tokens.black }}
        >
          Экспорт
        </button>
...
function MeetingInfo({
  ...
  // Пошеренная встреча целиком ридонли — теги только показываем
  const readOnly = m.authorId !== ME_ID;
  ...
  const openPicker = () => {
    ...
    if (row && plus)
      setPickerPos({
        left: plus.left - row.left,
        top: plus.bottom - row.top + 6,
      });
    setOpen(true);
  };
  ...
  const chip =
    "flex h-[23px] items-center gap-[4px] rounded-[3px] px-[8px] text-[12px] leading-[normal] tracking-[-0.24px]";

  return (
    <div className="flex w-full flex-col gap-[16px]">
      <h1
        className="text-[32px] font-semibold leading-[normal] tracking-[-0.96px]"
        style={{ color: tokens.black }}
      >
        {m.title}
      </h1>
      ...
      <div
        ref={rowRef}
        className="relative flex flex-wrap items-center gap-[6px]"
      >
        <span
          className={chip}
        ...
        >
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img
            src={sfAsset(source.icon)}
            alt=""
            className="h-[12px] w-[12px] max-w-none shrink-0 object-contain"
          />
          {source.label}
        </span>
        <span
          className={`${chip} gap-[8px]`}
          style={{ backgroundColor: tokens.bgSubtle, color: tokens.black }}
        >
          <span
            className="flex h-[16px] w-[16px] shrink-0 items-center justify-center rounded-full text-[9px] font-medium tracking-[-0.18px] text-white"
        ...
        {tags.map((t) =>
          readOnly ? (
            <TagChip key={t.id} name={t.name} color={t.color} size={23} />
          ) : (
            <span key={t.id} className="relative flex">
              <ChipWithMenu
                ...
                onDeleted={(tag, restore) => onDeleted(tag.name, restore)}
              />
            </span>
          ),
        )}
        ...
        <TagPicker
          ...
          anchorRef={btnRef}
          style={
            pickerPos ? { left: pickerPos.left, top: pickerPos.top } : undefined
          }
        />
      </div>
      <div className="flex w-full flex-col gap-[4px]">
        <span
          className="text-[13px] leading-[16px] tracking-[-0.13px] underline decoration-dotted underline-offset-[3px]"
          style={{ color: tokens.black, textDecorationColor: tokens.grey }}
        >
          Участники
        </span>
        <div className="flex w-full flex-wrap items-center gap-[2px]">
          {m.participants.map((name, i) => (
            <span key={name} className="flex items-center gap-[2px]">
              <span
                className="text-[13px] leading-[16px] tracking-[-0.13px] whitespace-nowrap"
...
function ArticleCard({
  ...
  return (
    <div
      className="flex w-full flex-col items-start gap-[12px] rounded-[4px] p-[16px]"
      style={{ backgroundColor: tokens.bgLight }}
    >
      <h2
        className="w-full text-[16px] font-medium leading-[normal] tracking-[-0.32px]"
        style={{ color: tokens.black }}
      >
        {title}
      </h2>
  ...
  return (
    <div className="flex w-full flex-col items-center gap-[16px] py-[24px]">
      <ArticleCard title="Краткое содержание:">
        <p
          className="w-full text-[13px] font-normal leading-[16px] tracking-[-0.13px]"
          style={{ color: tokens.black }}
        >
          {summary}
        </p>
      ...
      <ArticleCard title="Ключевые моменты:">
        <ul
          className="w-full list-disc pl-[20px] text-[13px] leading-[16px] tracking-[-0.13px]"
          style={{ color: tokens.black }}
        >
          {m.summary.map((s, i) => (
            <li key={i} className={i < m.summary.length - 1 ? "mb-[8px]" : ""}>
              ...
              >{`${i * 3 + 1}:${String((i * 17 + 8) % 60).padStart(2, "0")}`}</span>
            </li>
          ))}
        </ul>
      </ArticleCard>
      <ArticleCard title="Договоренности:">
        ...
        >
          <li className="mb-[8px]">
            Ответственные за пункты назначены, сроки — до следующей встречи.
          </li>
          <li className="mb-[8px]">
            Материалы по итогам разослать участникам сегодня.
          </li>
...
export default function TagsMeetingPage() {
  ...
  const stub = (label: string) =>
    toast.show(`Открываем «${label}»`, { icon: "arrow-right" });

  return (
    <main
      className={`${inter.className} h-screen min-h-[720px] w-full overflow-hidden bg-white`}
      style={{ color: tokens.black }}
    >
      <div className="flex h-full w-full bg-white">
        ...
        <section className="relative flex h-full min-w-0 flex-1 flex-col bg-white">
          <MeetingTopBar
            onAction={(l) =>
              l === "Ссылка скопирована" ? toast.show(l) : stub(l)
            }
          />
              ...
              {meeting && (
                <>
                  <div className="flex shrink-0 flex-col gap-[24px]">
                    <MeetingInfo
                      m={meeting}
                      onDeleted={(name, restore) =>
                        toast.show("Тег удален", { undo: restore })
                      }
                    />
