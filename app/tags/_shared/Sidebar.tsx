...
import { MODE_AVATAR_ART, MODE_GLYPH_16 } from "../../global-chat/_shared/mode-avatar-art";
import { ctaAsset } from "../../global-chat/_shared/tokens";
import { Popover, useOutsideClose } from "../../global-chat/_shared/ui";
import { WORKSPACES } from "./data";
import { focusRingClass, pressableClass, sbAsset, tokens } from "./tokens";
import { TgIc } from "./ui";
import { useTags } from "./use-tags";

// Сайдбар — из прототипа глобального чата (sidebar-menu-update + CTA из b2c-upgrade-cta).
// Для «Тегов» добавлен переключатель рабочего пространства в шапке: у каждого пространства
// свои встречи и свои теги, общие для всех его участников.
...
const primaryItems: Item[] = [
  ...
  { key: "meetings", label: "Встречи", icon: "meetings.svg", href: "/tags" },
...
/** Плашка пространства: буква на цветном фоне (картинка только у первого, из sidebar-menu-update) */
function WorkspaceMark({ id, initial, color, size = 28 }: { id: string; initial: string; color: string; size?: number }) {
  if (id === WORKSPACES[0].id) {
    // eslint-disable-next-line @next/next/no-img-element
    return <img src={sbAsset("workspace-avatar.png")} alt="" className="shrink-0 rounded-[3px] object-cover" style={{ width: size, height: size }} />;
  }
  return (
    <span className="flex shrink-0 items-center justify-center rounded-[3px] font-medium text-white" style={{ width: size, height: size, backgroundColor: color, fontSize: Math.round(size * 0.46), letterSpacing: "-0.26px" }}>
      {initial}
    </span>
  );
}

/** Шапка сайдбара: текущее пространство и меню переключения */
function WorkspaceSwitcher() {
  const { workspace, setWorkspace } = useTags();
  const [open, setOpen] = useState(false);
  const ref = useRef<HTMLDivElement>(null);
  useOutsideClose([ref], open, () => setOpen(false));
  return (
    <div ref={ref} className="relative flex h-[54px] w-full shrink-0 items-center border-b bg-white pl-[10px] pr-[16px]" style={{ borderColor: tokens.border }}>
      <button
        type="button"
        aria-haspopup="menu"
        aria-expanded={open}
        onClick={() => setOpen((v) => !v)}
        className={`flex h-[40px] shrink-0 items-center rounded-[4px] p-[6px] text-left hover:bg-[#F7F7F8] ${open ? "bg-[#F7F7F8]" : ""} ${pressableClass} ${focusRingClass}`}
      >
        <span className="flex items-center gap-[8px]">
          <WorkspaceMark id={workspace.id} initial={workspace.initial} color={workspace.color} />
      ...
      <Popover open={open} className="left-[10px] top-[50px] w-[248px]">
        <div role="menu" className="flex flex-col">
          {WORKSPACES.map((w) => {
            ...
            return (
              <button
                ...
                className={`flex h-[40px] w-full items-center justify-between gap-[8px] rounded-[3px] px-[6px] text-left hover:bg-[#F7F7F8] ${pressableClass} ${focusRingClass}`}
              >
                <span className="flex min-w-0 items-center gap-[8px]">
                  <WorkspaceMark id={w.id} initial={w.initial} color={w.color} size={24} />
                  <span className="truncate text-[13px] leading-[normal] tracking-[-0.13px]" style={{ color: tokens.black }}>
                    {w.name}
                  </span>
                </span>
                {on && (
          ...
          <p className="px-[6px] pb-[4px] pt-[6px] text-[12px] leading-[16px] tracking-[-0.24px]" style={{ color: tokens.grey }}>
            У каждого пространства свои теги, общие для всех участников
          </p>
        </div>
      </Popover>
    </div>
