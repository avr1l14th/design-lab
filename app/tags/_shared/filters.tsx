...
export const EMPTY_FILTERS: FilterState = { query: "", sources: [], authorIds: [], dateFrom: null, dateTo: null, tagIds: [] };

export function hasActiveFilters(f: FilterState): boolean {
  return f.sources.length > 0 || f.authorIds.length > 0 || f.dateFrom !== null || f.dateTo !== null || f.tagIds.length > 0;
}
...
/** Фильтр встреч: поиск по названию, источники, авторы, дата и теги (AND между типами) */
export function filterMeetings(all: TagMeeting[], f: FilterState, tagIdsOf: (meetingId: string) => string[]): TagMeeting[] {
...
function MainPanelRow({ icon, label, active, hasDot, onPick }: { icon: string | ReactNode; label: string; active: boolean; hasDot: boolean; onPick: () => void }) {
  return (
    <button
      type="button"
      role="menuitem"
...
function MainPanel({
  ...
  return (
    <div className="flex w-[200px] flex-col items-start rounded-[4px] bg-white p-[4px]" style={{ boxShadow: "0 0 4px 0 rgba(0,0,0,0.15)" }} role="menu">
      <MainPanelRow icon={<TgIc name="tag" />} label="Теги" active={activeTab === "tags"} hasDot={filters.tagIds.length > 0} onPick={() => onPick("tags")} />
          ...
          {tags.map((t) => (
            <button key={t.id} type="button" role="menuitemcheckbox" aria-checked={filters.tagIds.includes(t.id)} onClick={() => toggle(t.id)} className="flex h-[32px] w-full items-center gap-[8px] rounded-[3px] px-[8px] transition-colors hover:bg-[#F7F7F8]">
              <span className="flex min-w-0 items-center gap-[8px]">
                ...
                <span className="truncate text-[13px] font-normal" style={{ color: tokens.black, letterSpacing: "-0.13px" }}>
                  {t.name}
                </span>
              </span>
            </button>
