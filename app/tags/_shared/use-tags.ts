...
import { MEETINGS, SEED_STATE, TAG_COLORS, WORKSPACES, type Tag, type TagColor, type TagsState } from "./data";
...
function readStorage(): TagsState {
  try {
    ...
    if (!parsed || !Array.isArray(parsed.tags) || typeof parsed.assignments !== "object") return SEED_STATE;
    const workspaceId = WORKSPACES.some((w) => w.id === parsed.workspaceId) ? (parsed.workspaceId as string) : SEED_STATE.workspaceId;
    ...
    const tags = parsed.tags.map((t, i) => ({ ...t, color: t.color ?? TAG_COLORS[(i % (TAG_COLORS.length - 1)) + 1].id }));
...
export const cleanName = (name: string) => name.trim().replace(/\s+/g, " ").slice(0, TAG_NAME_MAX);
...
export function useTags() {
  const state = useSyncExternalStore(subscribe, () => getStore().value, () => SEED_STATE);

  const workspace = WORKSPACES.find((w) => w.id === state.workspaceId) ?? WORKSPACES[0];
  /** Теги текущего пространства по алфавиту */
  const tags = useMemo(() => state.tags.filter((t) => t.workspaceId === workspace.id).sort((a, b) => a.name.localeCompare(b.name, "ru")), [state.tags, workspace.id]);
  const tagById = useCallback((id: string) => state.tags.find((t) => t.id === id), [state.tags]);

  /** Теги встречи в порядке, в котором их ставили */
  const tagsFor = useCallback(
    (meetingId: string): Tag[] => (state.assignments[meetingId] ?? []).map((id) => state.tags.find((t) => t.id === id)).filter((t): t is Tag => Boolean(t)),
    [state.assignments, state.tags],
  );

  const findByName = useCallback((name: string) => tags.find((t) => normalizeName(t.name) === normalizeName(name)), [tags]);

  const usage = useCallback(
    (tagId: string): TagUsage => {
      let meetings = 0;
      for (const ids of Object.values(state.assignments)) if (ids.includes(tagId)) meetings += 1;
      return { meetings };
    },
    [state.assignments],
  );
  ...
  const tagsUsedIn = useCallback(
    (workspaceId: string): Tag[] => {
      ...
      return state.tags.filter((t) => t.workspaceId === workspaceId && used.has(t.id)).sort((a, b) => a.name.localeCompare(b.name, "ru"));
    },
    [state.assignments, state.tags],
  );
  ...
  const create = useCallback((rawName: string, assignTo?: string, color?: TagColor): Tag | null => {
    const name = cleanName(rawName);
    if (!name) return null;
    const s = getStore().value;
    const wsId = (assignTo && MEETINGS.find((m) => m.id === assignTo)?.workspaceId) || s.workspaceId;
    const wsTags = s.tags.filter((t) => t.workspaceId === wsId);
    const existing = wsTags.find((t) => normalizeName(t.name) === normalizeName(name));
    const palette = TAG_COLORS.slice(1);
    const autoColor = palette[wsTags.length % palette.length].id;
    const tag: Tag = existing ?? { id: nextId(), name, createdAt: Date.now(), color: color ?? autoColor, workspaceId: wsId };
    const tagsNext = existing ? s.tags : [...s.tags, tag];
    let assignments = s.assignments;
    if (assignTo) {
      const current = s.assignments[assignTo] ?? [];
      if (!current.includes(tag.id)) assignments = { ...s.assignments, [assignTo]: [...current, tag.id] };
    }
  ...
  const rename = useCallback((tagId: string, rawName: string): boolean => {
    ...
    const clash = s.tags.find((t) => t.id !== tagId && t.workspaceId === self?.workspaceId && normalizeName(t.name) === normalizeName(name));
    if (clash) return false;
    update({ ...s, tags: s.tags.map((t) => (t.id === tagId ? { ...t, name } : t)) });
    return true;
  }, []);

  const setColor = useCallback((tagId: string, color: TagColor) => {
    const s = getStore().value;
    update({ ...s, tags: s.tags.map((t) => (t.id === tagId ? { ...t, color } : t)) });
  }, []);
  ...
  const restore = useCallback((snapshot: RemovedTag) => {
    ...
    update({ ...s, tags: [...s.tags, snapshot.tag], assignments: { ...s.assignments, ...snapshot.assignments } });
  }, []);
  ...
  return { tags, tagById, tagsFor, findByName, usage, tagsUsedIn, toggle, create, rename, setColor, remove, restore, workspace, setWorkspace };
}
