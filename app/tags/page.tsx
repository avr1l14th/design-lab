...
function groupByDate(meetings: TagMeeting[]) {
  ...
  return Array.from(byDate.keys())
    ...
    .map((iso) => ({
      key: iso,
      ...dateLabels(iso),
      meetings: (byDate.get(iso) ?? []).sort((a, b) =>
        a.time < b.time ? 1 : -1,
      ),
    }));
}
...
function TagsCell({
  ...
  // Пошеренная встреча («Доступные мне») целиком ридонли — теги только показываем, без «+», крестиков и меню
  ...
  const measure = () => {
    ...
    setDirection(
      rect && window.innerHeight - rect.bottom < 360 ? "up" : "down",
    );
  };
  ...
  return (
    <div
      ...
      className="relative flex min-w-0 flex-1 items-center gap-[4px]"
    >
      ...
      <TagChipRow
        ...
        max={2}
        readOnly={readOnly}
        onMore={(e) => toggle(e, "more")}
        moreActive={openFrom === "more"}
      ...
      {!readOnly && (
        <AddTagChip
          onClick={(e) => toggle(e, "plus")}
          active={openFrom === "plus"}
...
function MeetingRow({
  ...
  return (
    <div className="group relative flex h-[72px] w-full items-center bg-white px-[24px] hover:bg-[#FAFAFA]">
      ...
      <Link
        href="/tags/meeting"
        onClick={() => rememberOpen(m.id)}
        aria-label={m.title}
        className={`absolute inset-0 ${focusRingClass}`}
      />
      <div className="pointer-events-none relative flex min-w-0 flex-1 items-center gap-[24px]">
        <span className="flex w-[446px] min-w-0 shrink-0 items-center gap-[12px]">
          ...
          <span className="flex min-w-0 flex-1 flex-col gap-[4px]">
            <span
              className="truncate text-[13px] font-medium leading-[normal] tracking-[-0.13px]"
              style={{ color: tokens.black }}
            >
              {m.title}
            </span>
            <span
              className="flex items-center gap-[4px] text-[12px] leading-[normal] tracking-[-0.24px]"
              style={{ color: tokens.grey }}
            >
              {m.time}
              <span
                className="h-[3px] w-[3px] rounded-full"
                style={{ backgroundColor: tokens.grey }}
              />
              {m.durationMin} мин
            </span>
          </span>
        ...
        <span className="flex w-[180px] shrink-0 items-center gap-[8px]">
          ...
          />
          <span
            className="min-w-0 flex-1 truncate text-[12px] leading-[normal] tracking-[-0.24px]"
            style={{ color: tokens.black }}
          >
            {author.email}
          </span>
        ...
        <span className="flex w-[160px] shrink-0 items-center gap-[8px] overflow-hidden">
          ...
          />
          <span
            className="truncate text-[12px] leading-[normal] tracking-[-0.24px]"
            style={{ color: tokens.black }}
          >
            {source.label}
          </span>
...
export default function TagsMeetingsPage() {
  ...
  const wsMeetings = useMemo(
    () => MEETINGS.filter((m) => m.workspaceId === api.workspace.id),
    [api.workspace.id],
  );
  ...
  const effectiveFilters = useMemo(() => {
    const alive = filters.tagIds.filter((id) =>
      api.tags.some((t) => t.id === id),
    );
    return alive.length === filters.tagIds.length
      ? filters
      : { ...filters, tagIds: alive };
  }, [filters, api.tags]);
  const list = useMemo(() => {
    const byTab = wsMeetings.filter((m) =>
      tab === "Мои встречи"
        ? m.authorId === ME_ID
        : tab === "Доступные мне"
          ? m.authorId !== ME_ID
    ...
    >
      <div className="flex h-full w-full bg-white">
        ...
        <section className="relative flex h-full min-w-0 flex-1 flex-col bg-white">
          <div
            className="flex h-[54px] w-full shrink-0 items-center border-b p-[16px]"
            style={{ borderColor: tokens.border }}
          >
            <h1
              className="text-[13px] font-medium leading-[normal] tracking-[-0.13px]"
              style={{ color: tokens.black }}
            >
              Встречи
            </h1>
          ...
          <div className="flex w-full shrink-0 items-center gap-[8px] px-[16px] py-[16px]">
            <div className="relative">
              <button
                ...
                onClick={() =>
                  filtersOpen ? closeFilters() : setFiltersOpen(true)
                }
              ...
              >
                ...
                <img
                  src={sfAsset(
                    active ? "icon-filter-active.svg" : "icon-filter.svg",
                  )}
                  alt=""
            ...
            {searchOpen ? (
              <label
                className={`flex h-[36px] w-[320px] items-center gap-[10px] rounded-[4px] border bg-white px-[10px] focus-within:border-[#0138C7] ${pressableClass}`}
                style={{ borderColor: tokens.border }}
              >
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img
                  src={sfAsset("icon-search.svg")}
                  alt=""
                  className="h-[16px] w-[16px] max-w-none shrink-0"
                />
                <input
            ...
            <div
              className="ml-[4px] h-[24px] w-px shrink-0"
              style={{ backgroundColor: tokens.border }}
            />
              ...
              {TABS.map((t) => {
                ...
                return (
                  <button
                    ...
                    style={{
                      color: on ? tokens.black : tokens.grey,
                      backgroundColor: on ? tokens.bgSubtle : undefined,
                    }}
                  >
                    {t}
                  </button>
            ...
            {groups.length === 0 ? (
              <div className="flex w-full flex-1 items-center justify-center py-[120px]">
                <div className="flex flex-col items-center gap-[12px] text-center">
                  <p
                    className="text-[24px] font-medium leading-[normal] tracking-[-0.48px]"
                    style={{ color: tokens.black }}
                  >
                    Не удалось ничего найти
                  </p>
                  <p
                    className="w-[288px] text-[14px] leading-[1.35] tracking-[-0.28px]"
                    style={{ color: tokens.black }}
                  >
                    {active
